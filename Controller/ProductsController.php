...
class productsController extends AppController{
	...
	function admin_edit($id = null) {
		if (!$id && empty($this->data)) {
			$this->Session->setFlash(__('Invalid product'), 'flash_error');
			$this->redirect(array('action' => 'index'));
		}
		if (!empty($this->data)) {
			pr($this->data);
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for product'), 'flash_error');
			$this->redirect(array('action'=>'index'));
		}
	...
	function admin_removeAvatar( $id = false ){
		$this->layout = 'ajax';
		$this->render(false);
		$this->Session->setFlash(__('Product image was not removed'), 'flash_error');
		if ($id) {
			...
			if ($this->Product->saveField('image', '')) {
				$this->Session->setFlash(__('Product image was removed'), 'flash_success');
			}
		}
