...
class SlideshowsController extends AppController {
	...
	function admin_add() {
		if (!empty($this->data)) {
			...
			if ($this->Slideshow->save($this->data)) {
				...
				$this->Session->setFlash(__('The slideshow has been saved'), 'flash_success');
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The slideshow could not be saved. Please, try again.'), 'flash_error');
			}
		}
	}
	...
	function admin_edit($id = null) {
		if (!$id && empty($this->data)) {
			$this->Session->setFlash(__('Invalid slideshow'), 'flash_error');
			$this->redirect(array('action' => 'index'));
		}
		if (!empty($this->data)) {
			unset($this->data['Slideshow']['image']);
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for slideshow'), 'flash_error');
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Slideshow->delete($id)) {
			$this->Session->setFlash(__('Slideshow deleted'), 'flash_success');
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Slideshow was not deleted'), 'flash_error');
		$this->redirect(array('action' => 'index'));
	}
	...
	function admin_removeAvatar( $id = false ){
		$this->layout = 'ajax';
		$this->render(false);
		$this->Session->setFlash(__('Image was not removed'), 'flash_error');
		if ($id) {
			...
			if ($this->Slideshow->saveField('image', '')) {
				$this->Session->setFlash(__('Image was removed'), 'flash_success');
			}
		}
