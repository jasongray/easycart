...
class UsersController extends AppController {
	...
	function admin_edit($id = null) {
		...
		if (empty($this->data)) {
			...
			if($this->Session->read('Auth.User.group_id') > $this->data['User']['group_id']){
				$this->Session->setFlash(__('You are not authorised to edit users who have a higher permission level than yourself.'), 'flash_error');
				$this->redirect(array('action' => 'index'));
			}
		}
		$userStatuses = $this->User->UserStatus->find('list');
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for user'), 'flash_error');
			$this->redirect(array('action'=>'index'));
		}
	...
	function admin_removeAvatar( $id = false ){
		$this->layout = 'ajax';
		$this->render(false);
		$this->Session->setFlash(__('User avatar was not removed'), 'flash_error');
		if ($id) {
			...
			if ($this->User->saveField('image', '')) {
				$this->Session->setFlash(__('User avatar was removed'), 'flash_success');
			}
		}
