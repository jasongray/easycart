...
class MenusController extends AppController{
	...
	function admin_add() {
		if (!empty($this->data)) {
			...
			if ($_id = $this->Menu->save($this->data)) {
				$this->Session->setFlash(__('The menu has been created'), 'flash_success');
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The menu could not be saved. Please, try again.'), 'flash_error');
			}
		}
	}
	...
	function admin_edit($id = null) {
		if (!$id && empty($this->data)) {
			$this->Session->setFlash(__('Invalid menu'), 'flash_error');
			$this->redirect(array('action' => 'index'));
		}
		if (!empty($this->data)) {
			unset($this->data['User']['image']);
			if ($this->Menu->save($this->data)) {
				$this->Session->setFlash(__('The menu has been saved'), 'flash_success');
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The menu could not be saved. Please, try again.'), 'flash_error');
			}
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for menu'), 'flash_error');
			$this->redirect(array('action'=>'index'));
		}
