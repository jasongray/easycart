...
if(!empty($this->data['Gallery']['id'])){
	$_title = __('Edit Gallery', true) . ' :: ' . $this->data['Gallery']['title'];
}else{
	$_title = __('Add Gallery', true);
}
...
<?php echo $this->Form->create('Gallery', array('id'=>'adminForm', 'class' => 'form label-inline'));?>
	<div class="portlet x4">
		<div class="portlet-header">
			<h4><?php echo __('Gallery', true);?></h4>
		</div>
...
<?php echo $this->Form->end(); ?>
	...
	<div id="gallery-wrapper" class="portlet x8" style="float:right;">
		...
		<?php foreach($images as $i){ ?>
				<li id="GImages-<?php echo $i['Image']['id'];?>">
					...
					<div class="actions">
					<?php
					echo $this->Html->link(__('Move', true), '#', array('class'=>'btn btn-orange btn-small btn-move', 'escape' => false));
					echo $this->Html->link(__('Delete', true), '/admin/galleries/removeImage/' . $i['Image']['id'], array('class'=>'btn btn-red btn-small cancelbtn', 'escape' => false));
					?>
					</div>
				</li>
		<?php } ?>
	...
	<div class="portlet x4" style="float:left;clear:left;">
		<div class="portlet-header">
			<h4><?php echo __('Upload Image', true);?></h4>
		</div>
		<div class="portlet-content">
