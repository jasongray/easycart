...
class ArosController extends AclAppController
{
	...
	function admin_update_user_role()
	{
	    ...
	    {
	        $this->Session->setFlash(__d('acl', 'The user role could not be updated'), 'flash_error', null, 'plugin_acl');
	    }
	...
	function admin_user_permissions($user_id = null)
	{
	    ...
	    else
	    {
            ...
            if(empty($user_aro))
            {
                ...
                $this->Session->setFlash(sprintf(__d('acl', "The user '%s' does not exist in the ARO table"), $display_user[$user_model_name][$user_display_field]), 'flash_error');
            }
            else
            {
            	$actions = $this->AclReflector->get_all_actions();
	...
	function admin_grant_all_controllers($role_id)
	{
        ...
        if(empty($node))
        {
            ...
            $this->Session->setFlash(sprintf(__d('acl', "The role '%s' does not exist in the ARO table"), $asked_role['Role'][Configure :: read('acl.aro.role.display_field')]), 'flash_error');
        }
        else
        {
            //Allow to everything
