...
?>\n";
				if (fwrite($handle, $txt) === FALSE) {
					$this->Session->setFlash(__('Could not save config information to file.', true), 'flash_warning');
					return false;
				}
				fclose($handle);
				$this->Session->setFlash(__('Config information saved.', true), 'flash_success');
				$this->redirect('/admin/users/dashboard');
			} else {
				$this->Session->setFlash(__('Config file is not writeable. Please correct the issue and try again!', true), 'flash_error');
				return false;
			}
		} else {
			$this->Session->setFlash(__('No data can be saved.', true), 'flash_error');
			return false;
		}
