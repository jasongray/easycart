...
class MenuItemsController extends AppController {
	...
	function admin_index() {
		...
		if (!$id) {
			$this->Session->setFlash(__('Invalid menu selected', true), 'flash_error');
	...
	function admin_add() {
		if (!empty($this->data)) {
			...
			if ($this->MenuItem->save($this->data)) {
				$this->Session->setFlash(__('The menu item has been saved', true), 'flash_success');
				$this->redirect(array('action' => 'index', 'menu_id' => $this->data['MenuItem']['menu_id']));
			} else {
				$this->Session->setFlash(__('The menu item could not be saved. Please, try again.', true), 'flash_error');
			}
		}
	...
	function admin_edit($id = null) {
		if (!$id && empty($this->data)) {
			$this->Session->setFlash(__('Invalid menu item', true), 'flash_error');
			$this->redirect(array('action' => 'index', 'menu_id' => $this->data['MenuItem']['menu_id']));
		}
		if (!empty($this->data)) {
			if($this->data['MenuItem']['controller'] === 'pages'){
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for menu item', true), 'flash_error');
			$this->redirect(array('action'=>'index', 'menu_id' => $this->params['named']['menu_id']));
		}
		if ($this->MenuItem->delete($id)) {
			$this->Session->setFlash(__('Menu item deleted', true), 'flash_success');
			$this->redirect(array('action'=>'index', 'menu_id' => $this->params['named']['menu_id']));
		}
		$this->Session->setFlash(__('Menu item was not deleted', true), 'flash_error');
		$this->redirect(array('action' => 'index', 'menu_id' => $this->params['named']['menu_id']));
	}
