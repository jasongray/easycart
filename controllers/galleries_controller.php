...
class GalleriesController extends AppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid gallery', true), 'flash_error');
	...
	function admin_add() {
		if (!empty($this->data)) {
			...
			if ($this->Gallery->save($this->data)) {
				$this->Session->setFlash(__('The gallery has been saved', true), 'flash_success');
				$this->redirect(array('action' => 'edit', $this->Gallery->id));
			} else {
				$this->Session->setFlash(__('The gallery could not be saved. Please, try again.', true), 'flash_error');
			}
		}
	}
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for gallery', true), 'flash_error');
			$this->redirect(array('action'=>'index'));
		}
		...
		if ($this->Gallery->delete($id)) {
			...
			$this->Session->setFlash(__('Gallery deleted', true), 'flash_success');
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Gallery was not deleted', true), 'flash_error');
		$this->redirect(array('action' => 'index'));
	}
		...
		if(!empty($this->data)){
			...
			if(move_uploaded_file($tempFile,$targetFile)){
				...
				if($this->Gallery->Image->save($this->data['Image'])){
					$this->Session->setFlash(__('Successfully added image to the gallery', true), 'flash_success');
					//$_result = '<script type="text/javascript">refreshGallery(); sortableImages();</script>';
				}else{
					$this->Session->setFlash(__('Failed to save file', true), 'flash_error');
				}
			}else{
				$this->Session->setFlash(__('Failed to move uploaded file', true), 'flash_error');
			}
		}
